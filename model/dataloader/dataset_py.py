...
class DataGenerator:
    def __init__(self, file_paths: list[Path], voca: list, chinese_only=True, negative_sample_rate:int=3, val_mode=False):
...
class TongShunDataset(IterableDataset):
    def __init__(self, file_paths: list[Path], voca: list, chinese_only=True, negative_sample_rate:int=3, tokenizer = lambda a:list(a), pad_len:tuple[int, int]=(30, 10), val_mode=False):
        '''
        negative_sample_rate: 负样本采样率，但由于每个字符会生成三条数据，所以实际负样本采样率为 negative_sample_rate/3
        voca: 输入法可用词库
        '''
        self.tokenizer = tokenizer
        self.pad_context_len, self.pad_x_len = pad_len

        self.random_queue = RandomQueue()
        self.generator = DataGenerator(file_paths, voca, chinese_only, negative_sample_rate, val_mode)
        self.finalized = self.finalize()
        for _ in repeat(None, 30):
            self.random_queue.add(next(self.finalized))

    def pad_context(self, string):
        return pad_list(string, self.pad_context_len, 1)

    def pad_x(self, string):
        return pad_list(string, self.pad_x_len, 1)
    ...
    def finalize(self):
        for i in self.generator:
            i = ((self.pad_context(self.tokenizer(i[0][0])), self.pad_x(self.tokenizer(i[0][1]))), i[1])
            i = (tensor(i[0][0]), tensor(i[0][1]), tensor(i[1]))
            yield i

    def shuffled(self):
        for i in self.finalized:
...
if __name__ == "__main__":
    ...
    data = TongShunDataset([Path("./test_data.txt")], voca, tokenizer=tokenizer.encode, val_mode=True)
    # data = TongShunDataset([Path("./test_data.txt")], voca)
